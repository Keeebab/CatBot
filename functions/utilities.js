module.exports = (client) => {
  ...
  client.checkConsent = async (client, message, msg) => {
    ...
    const channel = client.guilds.get('364865260097634305').channels.exists('topic', message.author.id);
    ...
    if (consent && channel) {
      ...
      if (agree.includes(response)) {
        ...
        const channel = (await client.guilds.get('364865260097634305').createChannel(message.author.tag.replace('#', '-').toLowerCase(), 'text')).setTopic("Client ID: " + message.author.id).then(c => { // eslint-disable-line no-unused-vars
          c.send({
            embed
          });
